    ...
    Pass {  
        CGPROGRAM
            ...
            half4 frag(v2f i) : COLOR {
                fixed4 game = tex2D(_GameTex, i.texcoordGame);
                fixed4 ambientLight = tex2D(_AmbientLightTex, i.texcoordAmbient);
                fixed4 lightSources = tex2D(_LightSourcesTex, i.texcoordLight)*_LightSourcesMul;
                fixed4 light = ambientLight + lightSources;

                fixed3 add = game.rgb / 2 + game.rgb * light.rgb * _LightMul;
                fixed3 bloom = (light.rgb + _AdditiveLightAdd)*pow(game.rgb, _AdditiveLightPow)*step(0.005, _AdditiveLightPow);
                fixed addSum = add.r + add.g + add.b;
                fixed prop = addSum / (game.rgb.r + game.rgb.g + game.rgb.b + addSum);
